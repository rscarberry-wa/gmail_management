...
from gmail_manager import GmailManager, date_range, CalendarTask
...
class AddCalendarTaskInput(BaseModel):
    """
    Input schema for the add_calendar_task_tool.
    """
    title: str = Field(
...
@tool(args_schema=AddCalendarTaskInput)
def add_calendar_task_tool(title: str, due: Optional[str] = None, notes: Optional[str] = None) -> str:
    """
    Add a new task to Google Calendar tasks.

    Args:
        title: The title of the task
        due: Due date for the task in YYYY-MM-DD format (optional)
        notes: Notes or description for the task (optional)
    ...
    try:
        ...
        success = GM.add_calendar_task(task)
        if success:
            return f"Task '{title}' added successfully"
        else:
            return f"Failed to add task '{title}' due to an API error"
    except Exception as e:
        logger.error(f"Error in add_calendar_task_tool: {str(e)}")
        return f"Failed to add task '{title}': {str(e)}"

class DeleteCalendarTaskInput(BaseModel):
    """
    Input schema for the delete_calendar_task_tool.
    """
    task_id: str = Field(
        description="The ID of the task to delete"
    )

@tool(args_schema=DeleteCalendarTaskInput)
def delete_calendar_task_tool(task_id: str) -> str:
    """
    Delete a task from Google Calendar tasks.
    ...
    """
    try:
        success = GM.delete_calendar_task(task_id)
        if success:
            return f"Task with ID {task_id} deleted successfully"
        else:
            return f"Failed to delete task with ID {task_id} due to an API error"
    except Exception as e:
        logger.error(f"Error in delete_calendar_task_tool: {str(e)}")
        return f"Failed to delete task with ID {task_id}: {str(e)}"

class UpdateCalendarTaskInput(BaseModel):
    """
    Input schema for the update_calendar_task_tool.
    """
    task_id: str = Field(
    ...
    status: str = Field(
        default="completed",
        description="The new status for the task (e.g., 'completed' or 'needsAction')"
    )

@tool(args_schema=UpdateCalendarTaskInput)
def update_calendar_task_tool(task_id: str, status: str = "completed") -> str:
    """
    Update the status of a task in Google Calendar tasks (e.g., mark as completed).

    Args:
        task_id: The ID of the task to update
        status: The new status for the task (default: 'completed')

    Returns:
        Confirmation message
    """
    try:
        success = GM.update_calendar_task(task_id, status=status)
        if success:
            return f"Task with ID {task_id} updated successfully to status '{status}'"
        else:
            return f"Failed to update task with ID {task_id} due to an API error"
    except Exception as e:
        logger.error(f"Error in update_calendar_task_tool: {str(e)}")
        return f"Failed to update task with ID {task_id}: {str(e)}"

class GetCalendarTasksInput(BaseModel):
    """
    Input schema for the get_calendar_tasks_tool.
    """
    start_date: str = Field(
        description="Start date for task retrieval in YYYY-MM-DD format"
    )
    end_date: str = Field(
        description="End date for task retrieval in YYYY-MM-DD format"
    )
    
@tool(args_schema=GetCalendarTasksInput)
def get_calendar_tasks_tool(start_date: str, end_date: str) -> str:
    """
    Retrieves tasks from Google Calendar tasks that are due between the specified start and end dates.

    Args:
        start_date: Start date for task retrieval in YYYY-MM-DD format
        end_date: End date for task retrieval in YYYY-MM-DD format

    Returns:
        Formatted summary of calendar tasks for the specified date range
    """
    try:
        # 1. Generate list of dates in the range
        dates = date_range(start_date, end_date)

        # 2. Retrieve tasks for these dates
        tasks = GM.get_calendar_tasks(dates)

        if not tasks:
            return f"No tasks found between {start_date} and {end_date}."

        # 3. Format tasks by date
        tasks_by_date = {}
        for task in tasks:
            if task.due:
                date_key = task.due.strftime("%Y-%m-%d")
                if date_key not in tasks_by_date:
                    tasks_by_date[date_key] = []
                tasks_by_date[date_key].append(task)

        # 4. Generate result string
        result = f"Calendar tasks from {start_date} to {end_date}:\n\n"
        
        # Sort dates for consistent output
        sorted_dates = sorted(tasks_by_date.keys())
        
        for date_key in sorted_dates:
            result += f"Tasks for {date_key}:\n"
            for task in tasks_by_date[date_key]:
                status_icon = "✓" if task.status == "completed" else "☐"
                result += f"  {status_icon} [ID: {task.id}] {task.title}\n"
                if task.notes:
        ...
        logger.error(f"Error in get_calendar_tasks_tool: {str(e)}")
...
if __name__ == "__main__":
    ...
    agent = create_agent(
        model=llm,
        tools=[fetch_emails_tool, reply_email_tool, send_new_email_tool, delete_email_tool,
               get_calendar_tasks_tool, delete_calendar_task_tool, update_calendar_task_tool,
        ...
        system_prompt=(
            ...
            "check calendar availability, schedule meetings, and add tasks."
        )
    )
    ...
    # message = HumanMessage(content=f"Get my calendar tasks during the time period 2026-04-23 to 2026-04-24. My email address is {os.getenv('EMAIL_ADDRESS')}")
    # message = HumanMessage(content=f"Add a task to my calendar to check the oil on my truck for 2026-04-24. My email address is {os.getenv('EMAIL_ADDRESS')}")
    message = HumanMessage(content=f"Mark task from my calendar with the ID dURMQ1FiY2h1bU5uaVRfZQ as complete. My email address is {os.getenv('EMAIL_ADDRESS')}")
